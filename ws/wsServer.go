...
func NewClient(conn *websocket.Conn, wsServer *WsServer, UUID string, deviceID string, topic string, platformCode string, versionCode string, apiVersion string) *Client {
	return &Client{
		Conn:         conn,
		WsServer:     wsServer,
		UUID:         UUID,
